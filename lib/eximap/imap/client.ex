defmodule Eximap.Imap.Client do
  ...
  def start_link(%{server: server, port: port, account: account, password: password}) do
  ...
  def init(state) do
    ...
    {:ok, socket} = Socket.connect(true, host, state.port, opts)
    ...
    req = Request.login(state.account, state.password) |> Request.add_tag("EX_LGN")
